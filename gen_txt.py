...
class Grid:
    def __init__(self, master, rows, cols):
        ...
        for i in range(rows):
            for j in range(cols):
                self.buttons[i][j] = tk.Button(master, width=2, height=1, bg='white', command=lambda i=i, j=j: self.click(i, j))
                self.buttons[i][j].grid(row=i+2, column=j)  # Start the grid on the third row
...
class Application:
    def __init__(self, master):
        ...
        self.entry_rows.grid(row=0, column=0, padx=5, pady=5)  # Add some padding around the entries
